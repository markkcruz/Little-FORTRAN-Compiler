...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (argc >= 2) {
        ...
        while (  ((t = getNextToken(finObj, lineNum)).GetToken() != DONE && t.GetToken() != ERR ) ){
            ...
            // for -v flag
            if (t.GetToken() == IDENT || t.GetToken() == PROGRAM || t.GetToken() == PRINT || t.GetToken() == READ || t.GetToken() == INTEGER || t.GetToken() == END || t.GetToken() == IF || t.GetToken() == THEN || t.GetToken() == REAL || t.GetToken() == CHAR || t.GetToken() == PLUS || t.GetToken() == MINUS || t.GetToken() == MULT || t.GetToken() == DIV || t.GetToken() == ASSOP || t.GetToken() == LPAREN || t.GetToken() == RPAREN || t.GetToken() == COMA || t.GetToken() == EQUAL || t.GetToken() == LTHAN || t.GetToken() == CONCAT || t.GetToken() == COLON ) {
                if (t.GetToken() == IDENT) {
    ...
    */
    ...
    if (_v) {
        ...
        for(auto & i : identKeyWord) // Now print it out
            std::cout << i << endl;
